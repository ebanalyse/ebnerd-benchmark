...
#
hparams.newsencoder_units_per_layer = [400, 400, 400]

# =============
print("Initiating articles...")
df_articles = pl.read_parquet(
    Path("~/ebnerd_data/eb_word2vec/eb_word2vec_tokens.parquet").expanduser()
)
df_articles = df_articles.with_columns(
    pl.col(df_articles.columns[-1]).list.head(MAX_TITLE_LENGTH)
)
word2vec_embedding = np.load(Path("~/ebnerd_data/eb_word2vec/vectors.npy").expanduser())
#
...
# ==
from ebrec.evaluation.metrics import roc_auc_score, ndcg_score, mrr_score
from tqdm import tqdm
import numpy as np


def compute_and_print_mean(aucs, mrrs, ndcg5s, ndcg10s):
    grouped_auc, grouped_mrr, grouped_ndcg5, grouped_ndcg10 = (
        np.mean(aucs).round(4),
        np.mean(mrrs).round(4),
        np.mean(ndcg5s).round(4),
        np.mean(ndcg10s).round(4),
    )
    print(f"AUC: {grouped_auc}")
    print(f"MRR: {grouped_mrr}")
    print(f"nDCG5: {grouped_ndcg5}")
    print(f"nDCG10: {grouped_ndcg10}")


df_labels = (
    pl.scan_parquet(PATH.joinpath("ebnerd_testset_gt", "test", "behaviors.parquet"))
    .select(DEFAULT_IMPRESSION_ID_COL, DEFAULT_LABELS_COL)
    .collect()
)

df = (
    df_labels.select(DEFAULT_IMPRESSION_ID_COL, DEFAULT_LABELS_COL)
    .join(df_test, on=DEFAULT_IMPRESSION_ID_COL, how="inner")
    .with_columns(pl.col("ranked_scores").list.eval(1 / pl.element()).alias("scores"))
)

aucs, mrr, ndcg5, ndcg10 = [], [], [], []

for i, row in enumerate(tqdm(df.iter_rows(named=True), ncols=80, total=df.height)):
    ground_truth = row["labels"]
    pred_score = row["scores"]
    aucs.append(roc_auc_score(y_true=ground_truth, y_score=pred_score))
    mrr.append(mrr_score(y_true=ground_truth, y_pred=pred_score))
    ndcg5.append(ndcg_score(y_true=ground_truth, y_pred=pred_score, k=5))
    ndcg10.append(ndcg_score(y_true=ground_truth, y_pred=pred_score, k=10))

    if i % 100_000 == 0:
        compute_and_print_mean(aucs, mrr, ndcg5, ndcg10)
